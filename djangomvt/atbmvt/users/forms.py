...
class CustomUserCreationForm(UserCreationForm):
    ...
    email = forms.EmailField(
        label="Email адрес",
        required=True,
        widget=forms.EmailInput(attrs={'class':'form-control', 'placeholder': "Вкажіть email адрес"})
    )
    ...
    last_name = forms.CharField(
        label="Прізвище",
        required=True,
        widget=forms.TextInput(attrs={'class':'form-control', 'placeholder': "Вкажіть прізвище"})
    )
    ...
    password1 = forms.CharField(
        label="Пароль",
        required=True,
        widget=forms.PasswordInput(attrs={'class':'form-control', 'placeholder': "Вкажіть пароль"})
    )
